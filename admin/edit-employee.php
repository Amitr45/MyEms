...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( empty($_REQUEST["desc"]) ){
                $descErr = "<p style='color:red'> * desc is required</p> ";
            }else{
                $desc = $_REQUEST["desc"];
            }
            ...
            if( !empty($name) && !empty($email) && !empty($pass) && !empty($salary) && !empty($desc)){
                ...
                if( mysqli_num_rows($r) > 0 ){
                    ...
                    $sql = "UPDATE employee SET `name` = '$name' , email = '$email', `password` ='$pass' , dob='$dob', gender='$gender' , salary='$salary' , `desc`='$desc' WHERE id = $_GET[id] ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                        echo "<script>
