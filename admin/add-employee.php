...
<?php  
        ...
        $nameErr = $emailErr = $passErr = $salaryErr= $descErr= "";
        $name = $email = $dob = $gender = $pass = $salary = $desc= "";
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( empty($_REQUEST["desc"]) ){
                $descErr = "<p style='color:red'> * Description is required</p> ";
            }else{
                $desc = $_REQUEST["desc"];
            }


            if( !empty($name) && !empty($email) && !empty($pass) && !empty($salary) && !empty($desc) ){
                ...
                if( mysqli_num_rows($r) > 0 ){
                    ...
                    $sql = "INSERT INTO employee(id, `name`, email, gender, dob, `password`, salary, dp, `desc`) VALUES (NULL,'$name','$email','$gender','$dob','$pass','$salary',NULL,'$desc')";
